/**
 * UserManagementPage — cross-hospital user management for super admin.
 * Lists all staff, allows role editing, real staff creation, and password reset.
 */
...
export function UserManagementPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["staff-all", page, search, roleFilter],
    queryFn: () => fetchStaff({
      page:     String(page),
  ...
  // Hospitals list for the create-staff modal dropdown
  ...
  const editForm = useForm({
    initialValues: { role: "" },
    validate: { role: (v) => v ? null : "Role is required" },
  });
  ...
  const editMutation = useMutation({
    ...
    onError: (err: unknown) => {
      notifications.show({ color: "red", message: formatApiError(err, "Failed to update role.") });
    },
  });
  ...
  const createForm = useForm({
    ...
    validate: {
      ...
      hospital_id: (v, values) =>
        values.role && values.role !== "super_admin" && !v
          ? "Hospital is required for non-super admin roles"
          : null,
  ...
  return (
    <Stack gap="lg">
      <Group justify="space-between">
        <Group gap="xs">
          ...
          <Box>
            ...
            <Text size="xs" c="dimmed">All staff across all hospitals · {data?.count ?? "—"} total users</Text>
          </Box>
        </Group>
      ...
      <Group gap="sm" wrap="wrap">
        ...
        />
        {(search || roleFilter) && (
          <Button variant="subtle" size="sm" onClick={() => { setSearch(""); setRole(null); setPage(1); }}>
            Clear
          </Button>
        )}
      </Group>
      ...
      {isLoading ? (
        ...
        <Box ta="center" py="xl"><Text c="dimmed">No users found.</Text></Box>
      ) : (
              ...
              <Table.Tbody>
                {staff.map((s) => (
                  <Table.Tr key={s.id}>
                    ...
                    <Table.Td><Text size="sm">{s.hospital?.name ?? "—"}</Text></Table.Td>
        ...
        <form onSubmit={editForm.onSubmit((v) => editMutation.mutate({ id: editStaff!.id, role: v.role as Role }))}>
          <Stack>
            ...
            <Group justify="flex-end">
              ...
              <Button type="submit" loading={editMutation.isPending} leftSection={<IconCheck size={16} />}>
                Save Role
              </Button>
            </Group>
          </Stack>
      ...
      <Modal opened={inviteOpen} onClose={closeInvite} title="Add Staff Member" centered>
        <form onSubmit={createForm.onSubmit((v) => createMutation.mutate(v))}>
          <Stack>
            ...
            <TextInput
              label="Email address"
              placeholder="kmensah@kath.edu.gh"
              type="email"
              required
